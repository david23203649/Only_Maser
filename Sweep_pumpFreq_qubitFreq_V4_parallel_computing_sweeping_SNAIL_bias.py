...
def steadysol_sweep_omega_s(Omega_s, tlist, args, g_over_delta = False ):
...
def steadysol_sweep_omega_p(Omega_p, Omega_s, tlist, args, g_over_delta):
    start = time.time()
    if __name__ == '__main__':
        ...
        for omega_p in tqdm(Omega_p, desc="Loading…", ascii=False, ncols=75):
            ...
            func = partial(steadysol_sweep_omega_s,tlist = tlist, args = args, g_over_delta = g_over_delta)
            result = parallel_map(func, Omega_s, progress_bar=True)
            #print(result)
    ...
    return steady_m,steady_q, steady_s, timeevo_m, timeevo_q, timeevo_s, Omega_p/two_pi/1e3, Omega_s/two_pi/1e3, tlist


#Omega_p = np.linspace(11.95,12.05,11)
Omega_p = np.linspace(11.4 * GHz * two_pi,13.5 * GHz * two_pi,101)
#Omega_q = np.linspace(6.96 * GHz * two_pi,7.2 * GHz * two_pi,25)
phase = two_pi * np.linspace(0.5, 0.7, 101)
Omega_s = (3.000 * abs(np.cos(phase)) + 3.500)*GHz* two_pi
...
for amp in amps:
    if __name__ == '__main__':
        ...
        nbar, qbar, sbar, timeevo_s, timeevo_q, timeevo_m, Omega_p, Omega_s, tlist = steadysol_sweep_omega_p(Omega_p, Omega_s, tlist, args, g_over_delta)
        ...
        #Save
        filepath = r"Z:\Data\Maser\Simulation\pythonProject1\Data\20240627\\"
        current_date = datetime.now().strftime('%Y-%m-%d')
        filename =  current_date + f"Masing_DAC_{amp}_g4_{args['g4']}_g_over_delta_{g_over_delta}_sweep_SNAIL_freq_bettew_res.ddh5"
    ...
    ## plot old
        ...
        # plt.plot(Omega_s, Omega_p,label = )
        # plt.colorbar(label='average photon number', fraction=0.046, pad=0.04)
        # plt.xticks(x_ticks, [custom_formatter(value, None) for value in Omega_s[x_ticks]])
        # plt.yticks(y_ticks, [custom_formatter(value, None) for value in Omega_p[y_ticks]])


    ## plot new
        plt.figure()
        plt.pcolormesh(Omega_s ,Omega_p,nbar)
        plt.plot(Omega_s,args['omega_m']/1000/(2*np.pi)+Omega_s)
        plt.xlabel('$\omega_s$(GHz)')
        plt.ylabel('$\omega_{p}$ (GHz)')
        plt.title(" average photon number sweeping SNAIL bias and pumping freq")
        plt.colorbar(label='average photon number', fraction=0.046, pad=0.04)
