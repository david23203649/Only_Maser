...
q_1 = operators.basis(0, 2)
m_1 = operators.basis(1, 2)
...
for alpha in tqdm(Alpha):
    ...
    eigenvalues_q0m2_matrix.append(eigenvalues_q0m2)  # Stack the q0m2 eigenvalues
    eigenvalues_q2m0_matrix.append(eigenvalues_q2m0)  # Stack the q2m0 eigenvalues
    ...
    # min_diff_array_transformed.append(min(diff_transformed))
...
# eigenvalues_q0m1_matrix = np.array(eigenvalues_q0m1_matrix).reshape(len(Alpha), len(Omega_q))
# eigenvalues_q1m0_matrix = np.array(eigenvalues_q1m0_matrix).reshape(len(Alpha), len(Omega_q))
eigenvalues_q0m2_matrix = np.array(eigenvalues_q0m2_matrix).reshape(len(Alpha), len(Omega_q))
eigenvalues_q2m0_matrix = np.array(eigenvalues_q2m0_matrix).reshape(len(Alpha), len(Omega_q))
...
non_zero_indices = [i for i, x in enumerate(min_diff_array) if x != 0]
min_diff_array_non_zero = [x for x in min_diff_array if x != 0]
Alpha_adjusted = Alpha[non_zero_indices]
#
Alpha_adjusted_1 = 10/Alpha_adjusted

# Plotting the updated figure
plt.figure()
plt.title('2 photon without transformation')
plt.plot(Alpha_adjusted, min_diff_array_non_zero, label = 'w/o transformation')
#plt.plot(Alpha_adjusted, Alpha_adjusted_1, label = '1/x')

plt.xlabel('alpha (MHz)')
plt.ylabel('avoid crossing (MHz)')
plt.grid(True)
# plt.yscale('log')
# plt.xscale('log')
plt.show()
...
plt.plot(phase1/two_pi, eigenvalues_q0m2_matrix[alpha_idx,:])
plt.plot(phase1/two_pi, eigenvalues_q2m0_matrix[alpha_idx,:])
